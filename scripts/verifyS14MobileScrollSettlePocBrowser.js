...
const productionViewportPath = resolve(distRoot, 'smoosic-editor', 'viewport-fit.js')
const candidates = [
...
<score-partwise version="4.0">
  <work><work-title>P2 Scroll Settle A/B</work-title></work>
...
function makeProofHtml(mode) {
  return `<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P2 ${mode} A/B</title></head>
<body data-p2-ab="pending" data-p2-mode="${mode}">
  ...
  <script>
    ...
    async function run() {
      ...
      const deltas = [0, 50, 100, 150, 200, 250];
...
const server = createServer((request, response) => {
  ...
  if (url.pathname === '/__p2-scroll-settle-ab.html') {
...
async function runMode(mode) {
  activeMode = mode
  const targetUrl = `http://127.0.0.1:${port}/__p2-scroll-settle-ab.html?mode=${mode}`
  const chromeArgs = [
    '--headless=new',
    '--no-sandbox',
    '--disable-gpu',
  ...
  const exitCode = await new Promise((resolveExit, rejectExit) => {
    ...
    const timeout = setTimeout(() => {
      child.kill('SIGKILL')
      rejectExit(new Error(`P2 ${mode} Chrome run timed out.`))
    }, 210000)
  ...
  if (exitCode !== 0) {
    throw new Error(`P2 ${mode} Chrome exit ${exitCode}: ${stderr.slice(-5000)}`)
  }
  ...
  if (!resultMatch) throw new Error(`P2 ${mode} browser evidence JSON missing.`)
  return JSON.parse(decodeHtmlText(resultMatch[1]))
}
...
const sharedSafety = (metrics) => (
  metrics.analysis.iframeReloadObserved === false &&
  metrics.analysis.sourceReimportObserved === false &&
  metrics.analysis.frameHiddenChanged === false
)
...
const evidence = {
  ...
  evidenceClass: 'CHROMIUM_REAL_BROWSER_AB_POC',
