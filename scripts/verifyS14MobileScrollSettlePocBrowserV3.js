...
if (!chrome) {
  console.error('P2 deterministic browser evidence failed closed: Chrome/Chromium not found.')
  process.exit(1)
}
...
for (const required of [
  ...
  if (!existsSync(required)) {
    console.error(`P2 deterministic browser evidence failed closed: missing ${required}`)
    process.exit(1)
  }
}
...
<score-partwise version="4.0">
  <work><work-title>P2 Scroll Settle A/B V3</work-title></work>
...
function makeProofHtml(mode) {
  return `<!doctype html>
<html lang="en">
<head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>P2 V3 ${mode}</title></head>
...
function assignMusicXml(win, doc) {
  ...
  const file = new win.File([xml], 'p2-scroll-settle-v3.musicxml', { type: 'application/vnd.recordare.musicxml+xml' });
...
async function run() {
  ...
  await waitFor(() => {
    ...
    return status.startsWith('Yüklendi:') && status.includes('p2-scroll-settle-v3.musicxml');
  ...
  function snapshot() {
    ...
    const expectedHeight = Math.floor(viewportTop + viewportHeight - rect.top - 4);
    return {
  ...
  spacer.id = 'p2-scroll-settle-v3-spacer';
  ...
  if (metrics.scroll.samples.length !== 6 || metrics.scroll.samples.at(-1)?.actual < 200) {
...
const server = createServer((request, response) => {
  ...
  if (url.pathname === '/__p2-scroll-settle-ab-v3.html') {
...
if (!port) {
  server.close()
  throw new Error('P2 V3 browser evidence could not acquire a local port.')
}
...
async function runMode(mode) {
  activeMode = mode
  const targetUrl = `http://127.0.0.1:${port}/__p2-scroll-settle-ab-v3.html?mode=${mode}`
  const args = [
...
const checks = {
  ...
  baselineIsProductionScript: baseline.initial.pocMode === '',
  pocScrollEstablished: poc.scroll.samples.length === 6 && poc.scroll.samples.at(-1).actual >= 200,
  pocSafetyPreserved: sharedSafety(poc),
  pocScriptLoaded: poc.initial.pocMode === 'scroll-settle-v2',
  pocTestHasMeaningfulGeometryChange: Math.abs(poc.afterBurst.expectedHeight - poc.initial.frameHeight) >= 1,
  ...
  pocAppliesExactSettledGeometry: Math.abs(poc.analysis.settledGeometryError) < 0.5,
  pocDatasetMatchesSettledGeometry: Number(poc.afterSettle.datasetHeight) === poc.afterBurst.expectedHeight,
  pocWritesExactlyOnceAfterSettle: poc.analysis.settleFrameWrites === 1,
  pocDoesNotWriteAgainWhenIdle: poc.analysis.idleFrameWrites === 0,
  pocDoesNotDriftWhenIdle: Math.abs(poc.analysis.idleGeometryDrift) < 0.5,
}
const failedChecks = Object.entries(checks).filter(([, ok]) => !ok).map(([name]) => name)
const evidence = {
  documentType: 'SmoosicMobileScrollSettleP2AbEvidence',
  evidenceClass: 'CHROMIUM_REAL_BROWSER_BASELINE_PLUS_DETERMINISTIC_POC_V3',
  productionBehaviorChanged: false,
  deterministicActualScrollVerified: true,
  productionViewportScriptReplacedOnlyInsidePocTestServer: true,
  note: 'Baseline timing is observational only; POC acceptance is gated by deterministic geometry/write/lifecycle invariants.',
  baseline,
  poc,
...
if (failedChecks.length) {
  console.error(`P2 V3 browser evidence failed checks: ${failedChecks.join(', ')}`)
